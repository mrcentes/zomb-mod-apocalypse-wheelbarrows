...
PFGMenu.addWorldContext = function(player, context, worldobjects, test)	
	...
	local function addItemsFromSquare(sq)
		if sq then
			local newItems = PFGMenu.getItems(sq)
	...
	-- Collect items from original and neighbors
	addItemsFromSquare(origSq)
	addItemsFromSquare(origSq:getN())
	addItemsFromSquare(origSq:getS())
